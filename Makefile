...
# шаг и добавляется в build-job, рядом с buf lint и buf generate. До тех пор
# несовместимое изменение контракта ничем не задерживается и уедет в main молча.
#
...
#
# Insecure-режима у сервера нет и не будет: вторая ветка исполнения,
...
dev-certs: ## Выпустить локальный CA и сертификаты сервера и product-svc для mTLS
	...
	@echo "Идентичность клиента: DNS SAN 'product-svc', её и ждёт backend."
