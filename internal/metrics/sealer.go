...
// WrapSealer считает отказы расшифрования client_uuid (§15).
//
// Место выбрано не «где удобно», а потому что Open — единственная воронка обоих
// путей расшифровки: диспетчер собирает payload перед RPC (§9), выдача ссылок
// строит VLESS URI (§8). Считать в них по отдельности значило бы забыть третий,
// когда он появится; комментарий в get_customer_access_links.go как раз ссылается
// на эту метрику как на отсутствующую.
//
// Seal не считается: его отказ означает сломанный ключ на записи, а §15 просит
// именно ошибки расшифрования. Провалившийся Seal к тому же валит команду целиком
