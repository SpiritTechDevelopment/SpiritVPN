...
// PruneUsageDedup — ретенция реестра дедупа usage-items.
//
...
// «начислим ровно один раз» ценой неограниченного роста. Поэтому записи,
// которые агент заведомо не пришлёт повторно, удаляются, а очень поздний повтор
// после такой очистки считается принятой положительной погрешностью.
//
...
// он защищает.
//
// Доменных решений у прунера нет: что именно безопасно удалять, знает
