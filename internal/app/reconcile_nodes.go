...
// на ноду, которая только что не ответила, нечего, а гнать его по
...
//
// Единственное место среза, где ошибка останавливает шаг целиком, а не
// пропускает проблемную запись. Набор авторитетен: юзер, выпавший из него, будет
...
// тот дрейф, ради которого reconcile существует; починенный молча, он не оставил
// бы следа, что вообще случается.
