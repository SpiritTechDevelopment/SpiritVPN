...
// Стабильные коды ошибок для structured logs.
//
...
// errorMapping — доменные исходы, их коды и сообщения; перечень исходов см. в
// шапке domain/errors.go.
//
...
// manifestErrorMapping — правила валидации манифеста и их коды.
//
...
// не попадёт. Вызывающий — infrastructure CI/CD, и деталь экономит ему
...
// statusFromManifestError отображает нарушение правил манифеста. Возвращает nil, если
...
//     тексту ошибки: одна и та же context.Canceled означает разное в
...
//  3. INTERNAL по умолчанию для всего неопознанного.
//
// Шаг 2 заполняет пробел: таблица доменных исходов обрыв стрима не
// рассматривает. Смысл шага в наблюдаемости.
// Штатный рестарт вызывающего с полусотней вызовов в полёте не должен
// выглядеть в метриках как полсотни отказов backend.
func statusFromError(ctx context.Context, err error) error {
	// Правила манифеста идут первыми и по тем же соображениям, что и доменные сентинелы
