...
--   * Никакого ON DELETE CASCADE: v1 не удаляет историю; FK нужны только
--     для ссылочной целостности строк, которые помечаются, а не удаляются.

CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- ---------------------------------------------------------------------------
-- Группа A. Проекция топологии (authority: infrastructure manifest)
-- ---------------------------------------------------------------------------
...
-- fleet может быть пустым. current=false — отсутствует в последнем снапшоте.
...
-- mutating-транзакцию и гейтит приём ReconcileUsers.
CREATE TABLE vpn_nodes (
    ...
    -- Состояние authoritative reconcile. Живёт здесь, а не отдельной
    ...
    -- нельзя удалять юзеров. Приезжает в GetNodeState, пишется pull
...
-- существующего routing_key. egress_tag передаётся агенту дословно как
-- User.egress_key.
...
-- ---------------------------------------------------------------------------
-- Группа B. Желаемое состояние customer (authority: backend)
-- ---------------------------------------------------------------------------
...
-- порядка: применяется только строго больший command_number. Эта строка —
-- точка сериализации SELECT ... FOR UPDATE для всех изменений одного customer.
...
-- отдельной таблицы блокировок нет.
...
-- вечно, а повторное добавление даёт новое поколение с новыми credentials.
...
-- ---------------------------------------------------------------------------
-- Группа C. Доставка agent-операций (transactional outbox + журнал исполнения)
-- ---------------------------------------------------------------------------
...
-- здесь НЕ хранится — он пересобирается из строки access прямо перед RPC.
-- lease_owner/lease_expires_at гейтят single-dispatch на ноду.
...
-- обрабатывается worker'ами пакетами, а не внутри RPC-транзакции manifest.
...
-- ---------------------------------------------------------------------------
-- Группа D. Учёт трафика через pull от агента
-- ---------------------------------------------------------------------------

-- Подтверждённая позиция usage-спула по ноде. Агент удаляет только подтверждённые
-- batches; новый spool_id сбрасывает sequence в ноль.
--
-- Она же lease-таблица pull worker'а: на ноду одновременно активен один
...
-- access_id / quota_period_id nullable для карантинных items.
...
-- item не блокировал batch. Только sanitized payload; никаких сырых секретов.
...
-- ---------------------------------------------------------------------------
-- Группа E. Аудит (append-only для runtime-роли)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Runtime-индексы сверх primary/unique ключей
-- ---------------------------------------------------------------------------
...
-- ограничение навсегда закрепило бы за ней её пару. Перенос route — удаление
-- старого и добавление нового routing_key — стал бы тогда
...
-- Путь выборки due customer для expiry worker.
--
...
--
-- Он же обслуживает метрику возраста самой старой записи: min(processed_at)
