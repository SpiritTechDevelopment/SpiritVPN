...
const (
	...
	BlockReasonAdministrative        BlockReason = "ADMINISTRATIVE_BLOCK"
	BlockReasonDeleting              BlockReason = "DELETION_IN_PROGRESS"
)
...
//
//  1. DELETING и административный BLOCKED имеют приоритет над автоматическими
//     причинами;
//  2. истёкший срок действует на всего customer сразу;
//  3. исчерпанная квота — только на его access на этой ноде. Порядок 2 и 3
...
//     показать;
//  4. недоставленная навсегда операция и непригодная входная нода одинаково
//     означают «ссылка сама не появится» — в отличие от PENDING, который
//     обещает, что появится;
//  5. READY — ровно конъюнкция условий пригодности за вычетом уже проверенного выше и за вычетом
//     присутствия цели в текущем manifest: отсутствующая цель до этой функции не
//     доходит, её отсекает запрос;
//  6. остальное — PENDING: PENDING и RETRYING доставки, а также ABSENT без
...
func LinkStatusOf(in LinkInput) LinkStatus {
	switch {
	case in.Lifecycle == CustomerLifecycleDeleting:
		return LinkStatus{State: LinkStateBlocked, Reason: BlockReasonDeleting}
	case in.Lifecycle == CustomerLifecycleBlocked:
		return LinkStatus{State: LinkStateBlocked, Reason: BlockReasonAdministrative}
