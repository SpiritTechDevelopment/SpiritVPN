...
// того, что desired state с тех пор сменился. Меняется только RETRY_WAIT: повтор
// операции устаревшей версии толкнул бы на ноду заведомо неверное состояние,
// поэтому она становится терминальной SUPERSEDED.
//
// ApplyState в возвращённом плане не обнуляется и не используется: применить его
// к строке access всё равно нельзя, и записывающий слой знает это сам.
