...
const statsAccesses = `-- name: StatsAccesses :many
...
func (q *Queries) StatsAccesses(ctx context.Context) ([]StatsAccessesRow, error) {
	rows, err := q.db.Query(ctx, statsAccesses)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []StatsAccessesRow{}
	for rows.Next() {
		var i StatsAccessesRow
		if err := rows.Scan(&i.DesiredState, &i.ApplyState, &i.Count); err != nil {
			return nil, err
		}
...
const statsAgentOperations = `-- name: StatsAgentOperations :many
...
type StatsAgentOperationsRow struct {
	Status           string
	Count            int64
...
func (q *Queries) StatsAgentOperations(ctx context.Context) ([]StatsAgentOperationsRow, error) {
	rows, err := q.db.Query(ctx, statsAgentOperations)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []StatsAgentOperationsRow{}
	for rows.Next() {
		var i StatsAgentOperationsRow
...
const statsNodeCursors = `-- name: StatsNodeCursors :many
SELECT c.node_id,
...
type StatsNodeCursorsRow struct {
	NodeID             string
...
func (q *Queries) StatsNodeCursors(ctx context.Context) ([]StatsNodeCursorsRow, error) {
	rows, err := q.db.Query(ctx, statsNodeCursors)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []StatsNodeCursorsRow{}
	for rows.Next() {
		var i StatsNodeCursorsRow
		if err := rows.Scan(
			&i.NodeID,
			&i.LastPullAgeSeconds,
			&i.AckedSequence,
			&i.LeaseExpired,
		); err != nil {
			return nil, err
...
const statsQuarantine = `-- name: StatsQuarantine :many
SELECT reason, count(*)::bigint AS count
FROM traffic_batch_quarantine
GROUP BY reason
`

type StatsQuarantineRow struct {
	Reason string
	Count  int64
}
...
func (q *Queries) StatsQuarantine(ctx context.Context) ([]StatsQuarantineRow, error) {
	rows, err := q.db.Query(ctx, statsQuarantine)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []StatsQuarantineRow{}
	for rows.Next() {
		var i StatsQuarantineRow
...
const statsScalars = `-- name: StatsScalars :one
WITH due AS (
...
func (q *Queries) StatsScalars(ctx context.Context) (StatsScalarsRow, error) {
	row := q.db.QueryRow(ctx, statsScalars)
	var i StatsScalarsRow
	err := row.Scan(
		&i.ManifestRevision,
		&i.MaterializedRevision,
		&i.MaterializationLagSeconds,
		&i.ExpiredCustomers,
		&i.ExpiryLagSeconds,
		&i.ExhaustedNodeQuotas,
		&i.DispatchLeasesHeld,
		&i.DispatchLeasesExpired,
		&i.MaterializeLeasesHeld,
