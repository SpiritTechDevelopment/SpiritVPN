-- Снимок состояния БД для метрик.
--
...
-- Agent operations по статусу: сколько и насколько стара самая старая.
--
...
-- Access по паре состояний.
--
-- Ретайрнутые исключены: они уже не часть желаемого состояния, и их apply_state —
-- история. Индекса под эту группировку нет и не заводится: на масштабе v1
...
-- Состояние опроса по каждой текущей ноде.
--
...
-- Карантинные usage-items по причине.
--
...
SELECT
    ...
    -- Возраст самой старой дедуп-записи — наблюдаемость ретенции.
    --
